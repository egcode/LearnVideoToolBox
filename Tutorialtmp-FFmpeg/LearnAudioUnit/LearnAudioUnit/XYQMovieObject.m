...
- (BOOL)initializeResources:(const char *)filePath {
    ...
    if (avformat_open_input(&XYQFormatCtx, filePath, NULL, NULL) != 0) {
        NSLog(@"fail to open the file");
        goto initError;
    }
    // Check data flow
    if (avformat_find_stream_info(XYQFormatCtx, NULL) < 0) {
        NSLog(@"Check data flow failed");
        goto initError;
    }
    // Find the first video stream based on the data stream
    if ((videoStream =  av_find_best_stream(XYQFormatCtx, AVMEDIA_TYPE_VIDEO, -1, -1, &pCodec, 0)) < 0) {
        NSLog(@"Did not find the first video stream");
        goto initError;
    }
...
#if DEBUG
    // Print the details of the video stream
    ...
    if (pCodec == NULL) {
        NSLog(@"No decoder found");
        goto initError;
    }
    // Open the decoder
    if(avcodec_open2(XYQCodecCtx, pCodec, NULL) < 0) {
        NSLog(@"Failed to open the decoder");
        goto initError;
    }
    // Assign video frames
...
#pragma mark ------------------------------------
#pragma mark  Override attribute access method
...
//    memcpy(base, frame->data[1], bytesPerRowUV * frame->height / 2);
    ...
    /*
     The premise here is that the format of yuv in AVFrame is nv12;
           But if the AVFrame is yuv420p, you need to store each byte of frame->data[1] and frame->data[2] on the plane1 of the pixelBUffer, that is, save the original uuuu and vvvv as uvuvuvuv.
     */
