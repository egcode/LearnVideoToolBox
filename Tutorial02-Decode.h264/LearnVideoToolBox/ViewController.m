...
- (void)viewDidLoad {
    ...
    self.mLabel.text = @"Test H264 hard decoding";
    ...
    self.mDispalyLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(updateFrame)];
        self.mDispalyLink.frameInterval = 2; // The default is 30FPS frame rate recording
...
- (void)readPacket {
    ...
    if (memcmp(inputBuffer, lyStartCode, 4) == 0) {
        if (inputSize > 4) { //In addition to the start code and content
            uint8_t *pStart = inputBuffer + 4;
            uint8_t *pEnd = inputBuffer + inputSize;
            while (pStart != pEnd) { //Here is an abbreviated way to get the length of this frame: by looking for the next 0x00000001.
                if(memcmp(pStart - 3, lyStartCode, 4) == 0) {
                    ...
                    memcpy(packetBuffer, inputBuffer, packetSize); //Copy the contents of the packet to the new buffer
                    memmove(inputBuffer, inputBuffer + packetSize, inputSize - packetSize); //Move the buffer forward
    ...
    if (inputStream){
        dispatch_sync(mDecodeQueue, ^{
            ...
            *pNalSize = CFSwapInt32HostToBig(nalSize);
            ...
            // Fill in the length of the buffer in front of the length of the int
    ...
    if (mDecodeSession) {
        ...
        if(status == kCMBlockBufferNoErr) {
            ...
            if (status == kCMBlockBufferNoErr && sampleBuffer) {
                ...
                // Call didDecompress, return and then callback
